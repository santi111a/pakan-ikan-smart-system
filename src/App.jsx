...
function App() {
  ...
  const [hidroInput, setHidroInput] = useState({ tglTanam: '', namaTanaman: '', jenisNutrisi: '', statusHama: '' });
  ...
  const handleSimpanHidro = () => {
    if (!hidroInput.tglTanam || !hidroInput.namaTanaman) return alert("Mohon isi minimal tanggal dan nama tanaman!");
    push(ref(db, 'jurnal_hidroponik'), hidroInput).then(() => {
      alert("✅ Jurnal Hidroponik Tersimpan!");
      setHidroInput({ tglTanam: '', namaTanaman: '', jenisNutrisi: '', statusHama: '' });
    });
  };
      ...
      <div style={{ flex: 1, padding: '40px', overflowY: 'auto' }}>
        ...
        {halaman === 'beranda' && (
          <div style={{ maxWidth: '900px', margin: '0 auto' }}>
            <h1 style={{ color: '#38bdf8', marginBottom: '10px', fontSize: '36px' }}>Ringkasan Sistem</h1>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '20px', marginBottom: '40px' }}>
              <div style={cardStyle}><h4 style={cardLabel}>STATUS PAKAN</h4><h2 style={cardValue}>{data.jam_sore}:00</h2><p style={{ color: '#64748b', fontSize: '12px' }}>Sore Hari</p></div>
              
              {/* LOG UDARA DI BERANDA */}
              <div style={cardStyle}>
                <h4 style={cardLabel}>LOG UDARA / KIPAS</h4>
                <h2 style={{ ...cardValue, color: data.kipas_on ? '#22c55e' : '#ef4444' }}>{data.kipas_on ? 'AKTIF' : 'MATI'}</h2>
                <p style={{ color: '#64748b', fontSize: '12px' }}>Kondisi: {data.kipas_on ? 'Suhu Panas' : 'Suhu Normal'}</p>
              </div>

              <div style={cardStyle}><h4 style={cardLabel}>HIDROPONIK</h4><h2 style={cardValue}>PANEN</h2><p style={{ color: '#64748b', fontSize: '12px' }}>Cek Jurnal berkala</p></div>
            </div>
          </div>
        )}
        ...
        {halaman === 'pakan' && (
          <div style={{ display: 'flex', justifyContent: 'center' }}>
            <div style={formContainer}>
              <h2 style={{ color: '#38bdf8', textAlign: 'center', fontSize: '26px', marginBottom: '20px' }}>Pengaturan Pakan</h2>
              <div style={{ textAlign: 'left' }}>
                ...
                <button onClick={handleUpdate} style={updateBtnStyle}>UPDATE SISTEM</button>
              </div>
            </div>
          </div>
        )}
          ...
          <div style={{ maxWidth: '900px', margin: '0 auto' }}>
            ...
            <div style={historyBox}>
               <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead><tr style={{ borderBottom: '1px solid #334155', color: '#38bdf8' }}><th style={thStyle}>Tgl</th><th style={thStyle}>Jumlah</th><th style={thStyle}>Ukuran</th></tr></thead>
                <tbody>{listJurnal.map((item) => (<tr key={item.id} style={{ borderBottom: '1px solid #1e293b' }}><td style={tdStyle}>{item.tglBibit}</td><td style={tdStyle}>{item.jumlahIkan}</td><td style={tdStyle}>{item.ukuranBibit}</td></tr>))}</tbody>
              </table>
            </div>
          </div>
        )}
